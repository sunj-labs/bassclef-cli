...
iteration_bet: cold-adopter dry-run fix + trusted-publisher unblock + boundary alignment (SHIPPED as @thebassclef/lite@0.1.3 on 2026-09-12)
iteration_started: 2026-09-12
session: docs/session-logs/2026-09-12-dry-run-fix-plus-trusted-publisher-plus-boundary-align.md
iteration_phase: **SHIPPED + LIVE — @thebassclef/lite@0.1.3 published to npm 2026-09-12 via trusted publisher (OIDC + provenance).** First OIDC-authed publish for `@thebassclef/lite`; trusted publisher UI unblocked mid-session after prior 404s cleared. Session shipped 2 merged PRs + 6 filed issues + cross-session boundary alignment with bassclef-upstream. PR #61 (fix #60 dry-run parity, 81c8ed6) + PR #65 (release v0.1.3, 4bf8f15). Registry: [0.0.1, 0.1.0, 0.1.2, 0.1.3]. Provenance link: npmjs.com/package/@thebassclef/lite/v/0.1.3#provenance. Cold-adopter smoke on live 0.1.3 install confirmed all three #60 defects fixed end-to-end. Cross-session dialogue with bassclef-upstream converged on hybrid Path 1 + Path 2 for the hooks: {} class per ADR-051 D1; cli-side one-PR fix waits on upstream 12e (wiring manifest tier extension + dist/<tier>/ tree).
...
open_threads:
  ...
  - **#66 — Node 20 → 22 bump in workflows** (filed 2026-09-12). GHA deprecation warning surfaced on v0.1.3 publish run. Small mechanical bump, ~10-15 turns. Not urgent.
...
2026-09-12 session shipped `@thebassclef/lite@0.1.3` live via trusted publisher (OIDC + provenance) — first non-manual publish since the trusted-publisher UI unblocked mid-session. Session ran ~120 turns across ~5 hours. Cold-adopter test on isolated macOS profile surfaced #60 — `bassclef init --dry-run` reported 2 files while real run wrote 283 (~140x under-report). Root cause at `src/commands/init.ts:125-127` — dry-run returned early before invoking `dispatchSubstrateCopy`; `copySubstrate` already had the dry-run gate at `src/lib/copy-substrate.ts:51,142-145` but never got called with the flag. Two-line wiring fix shipped as PR #61 (81c8ed6) with Beck RED/GREEN Tier 0 test + counter unification + `--help` rewrite. Bumped 0.1.2 → 0.1.3 as PR #65 (4bf8f15); tagged v0.1.3; GitHub Release fired the publish workflow; operator approved environment gate; publish landed. Cold-adopter smoke on live 0.1.3 install confirmed all three fix defects end-to-end (282 would-create lines matching manifest; unified counter reads "282 files total (2 config + 280 substrate)"). Trusted publisher path: pursued NPM_TOKEN fallback (Option C) after `/diagnose` of the 404, then closed PR #64 + #62 + #63 when the setup UI loaded successfully — configured GitHub Actions publisher pointing at publish.yml + npm-publish environment. Also filed #66 (Node 20 → 22 bump; GHA deprecation warning) + #67 (session-start prompt for auto-sync when behind; Discovery Cascade Layer 2). Cross-session dialogue with bassclef-upstream surfaced a bigger class — `settings.json` ships `hooks: {}` empty at init time; 14+ vendored hooks dead-letter on every cold adopter since 0.1.0. Root cause at `src/commands/init-templates/settings-json.ts:27` — stale 0.0.x comment that predates bundled-substrate shipping (scope-b1 PR #36). Boundary converged on hybrid Path 1 + Path 2 per ADR-051 D1 — upstream owns the wiring manifest with per-entry tier tags; build step writes per-tier `dist/<tier>/` tree; cli reads verbatim. Cli-side one-PR fix (bassclef-cli#68) waits on upstream 12e shipping the dist/<tier>/ contract. Upstream files 4 tickets tonight (#1607, #1608, #1609, #68 mirror). Trusted publisher config also validated npm's Aug 2026 / Jan 2027 token-restriction path — long-term direction locked. All 229/229 tests green throughout.
...
2026-09-12T16:45:00Z — session-end (@thebassclef/lite@0.1.3 shipped live to npm via trusted publisher OIDC + provenance; PR #61 fixed #60 dry-run parity; PR #65 released 0.1.3; PR #64 closed superseded; issues #62 + #63 closed superseded; issues #66 + #67 + #68 filed; cross-session boundary alignment with bassclef-upstream on hybrid Path 1 + Path 2 per ADR-051 D1)
session: docs/session-logs/2026-09-12-dry-run-fix-plus-trusted-publisher-plus-boundary-align.md
