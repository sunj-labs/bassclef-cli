...
iteration_bet: cli#73 Phase 3 + Phase 4-agent — init walker reads dist/lite/, drops substrate/, MAJOR 1.0.0 (SHIPPED as PR #76 → main commit c974f33 on 2026-09-13; version 1.0.0 MAJOR)
iteration_started: 2026-09-13
session: docs/session-logs/2026-09-13c-cli-73-phase-3-init-walker.md
iteration_phase: **SHIPPED — cli#73 Phase 3 merged to main via PR #76 (c974f33) at 2026-09-13T02:15:05Z. Version bumped 0.2.0 → 1.0.0 MAJOR.** Six commits on `feat/cli-73-phase-3-init-walker` squashed to one merge commit. Init walker reads dist/lite/ per ADR-055 D1. Fails loud with exit 4 (manifest missing) + exit 5 (schema mismatch) per D4. Prints `N hooks armed (<tier> tier)` banner per D5. Substitutes `[REPO_NAME]` + `[ISO_TIMESTAMP]` + `[TIER]` in CLAUDE.md + whereami.md + .bassclef-source.json. substrate/ bundle path retired (operator confirmed zero npm adopters; no compat-shim owed per ADR-031 threshold). ADR-001 §Invariants amended (dist/lite/** sole substrate path). ADR-002 §Amendment 2026-09-13 already carried the new exit codes. Suite 224/224 GREEN. Phase 4-agent tarball smoke PASS (exit 0 happy path + exit 4 missing manifest + exit 5 schema mismatch + placeholders substituted). Session ran ~95 turns. Ticket #73 closes with this ship.

open_threads:
  - **Phase 4-operator cold-adopter smoke** — pending morning operator smoke on cold-adopter-1 profile. Steps: open Claude Code in fresh profile with @thebassclef/lite@1.0.0 installed, verify hooks fire at SessionStart, confirm N hooks armed banner. Blocks publish dispatch.
  - **Publish 1.0.0 to npm** — MAJOR bump gates on operator morning smoke. Dispatch publish workflow (release-published trigger with v1.0.0 tag OR workflow_dispatch). Zero adopters today, so upgrade break is null.
  - **CLAUDE.md template placeholder mismatch** — upstream template ships literal `# [Repo name]` text (lowercase, with space) but cli substitutes only `[REPO_NAME]` (all caps). Adopters see literal `[Repo name]` in their CLAUDE.md. Follow-on to file at bassclef-upstream for template rewrite OR at bassclef-cli for regex extension.
