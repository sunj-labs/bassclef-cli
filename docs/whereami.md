...
last_updated: 2026-09-20T10:55:00Z
iteration_bet: cli 1.2.1 shipped to npm 2026-09-19T23:26 with v0.46.0 substrate bundle; cold-adopter smoke surfaced 12 hooks wired-but-missing; launch delayed 24h to 2026-09-21 pending upstream cures (4 p0 blockers filed: bassclef-upstream#1822, #1824, #1825, #1827); Docker harness plan filed at cli#162 for post-launch smoke isolation
iteration_started: 2026-09-19
session: docs/session-logs/2026-09-20a-launch-eve-smoke-blockers-plus-docker-plan.md (~12h; launch-eve smoke + audit + tickets)

**operator_recap 2026-09-20a:** Launch-eve session. Cli 1.2.1 shipped to npm cleanly (publish workflow green, sigstore provenance published). Cold-adopter smoke on 1.2.1 reported 37/1 pass but the pass count was polluted by peer clone at `~/tmp/bassclef/` overlaying operator substrate onto the "cold" install. Real cold-adopter simulation via account swap (kingofrock → giveisusfree) confirmed the true blast radius — cli 1.2.1 is NOT self-contained. Substrate bundle wires 12 hooks in `settings.json` whose source files do not ship in the lite tier. Every bash tool call in a real adopter session fires 6 PreToolUse errors on those missing files. Root cause: lite manifest generator emits settings entries for hooks it does not also emit as files. Filed 11 upstream tickets (bassclef-upstream#1815-#1828) + 3 cli tickets (#160 add /onboard-repo to smoke drive; PR #161 new settings-hooks-present static check script; #162 Docker harness plan). Also deleted 2 tickets (cli#156, #157) that had identifier leaks. Launch decision Option C — wait for upstream cures — moves ship to 2026-09-21. New script at `scripts/smoke-assert-settings-hooks.sh` is the static complement that would have caught the 12-hook class if it had existed pre-1.2.1. Docker harness plan (cli#162) grounded via `/extract-intent` LIVE at 0.92 confidence — Cockburn walking skeleton + Beck tests-as-first-client + Zeller hypothesis-test. Post-launch that becomes the primary smoke lane, taking operator machine out of the test path.
