...
last_updated: 2026-09-20T20:52:00Z
in_flight_goal: goal 2026-09-20e — cold-adopter harness findings dialogue with bassclef-upstream. Ran docker harness against `@thebassclef/lite@1.2.2` on npm; traced the 25-file user-scope drop end-to-end through `src/lib/scope-router.ts` + `src/lib/copy-substrate.ts` + `src/commands/init.ts`; filed cli#177 (dual-write doc-only cure — Finding 5 (a)) + cli#178 (additive banner line for 23 undeclared helpers — Finding 2). Ran the harness end-to-end 4 times across the session; final exit 0 on `@thebassclef/lite@1.2.2`. New `scripts/smoke-drive-onboard-repo.sh` + `harness/docker/entry.sh` V2 Step 6 shipped (uncommitted at session-end save-state). Upstream Session A tomorrow lands Deliverable 1 rewrite verbatim into `docs/designs/2026-09-20-bassclef-sync-adopter-vs-operator.md` L96-102 + files A1-A6 mirrors + fires v2.0.0 release cascade.
iteration_started: 2026-09-20
session: docs/session-logs/2026-09-20e-cold-adopter-harness-findings-dialogue.md
