...
next_bet: pick from (a) Epic #194 Story 4 approval-gate notifications (via ticket #192; requires operator to enable GitHub deployment notifications + pick secondary path); (b) 3-part smoke-drive extension for /riff + /launch + /build (Q1 pending); (c) wait for v1.1.2 peer signal + sync + merge PR #181 statusline; (d) ticket #193 env modernization (Node 22 + ubuntu-24.04 pins).
